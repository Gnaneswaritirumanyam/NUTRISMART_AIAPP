...
class NotificationManager {
    constructor() {
        ...
        this.isNative = !!(window.Android && window.Android.scheduleMealPlan);
    ...
    async requestPermission() {
        if (this.isNative) return true; // Android permissions handled natively
        if (this.isCapacitor) {
    ...
    async schedulePlanNotifications(plan) {
        ...
        Object.entries(mealTimes).forEach(([mealType, timeStr]) => {
            ...
            if (this.isNative) {
                const email = plan.userId || "user@example.com";
                window.Android.scheduleMealPlan(email, plan.planType, mealType, hours, minutes);
                // The Android side handles both the meal notification and the 1-hour feedback delay
                return;
            }
