...
class PlanManager {
    ...
    async disableActivePlan() {
        ...
        // Unlock elements dynamically instead of reloading
        const elementsToEnable = document.querySelectorAll('input:not([id*="Time"]), select, textarea, button[id*="generate"]');
        ...
        if (generatorPanel && generatorPanel.firstChild && generatorPanel.firstChild.innerHTML && generatorPanel.firstChild.innerHTML.includes("Form Locked")) {
            generatorPanel.removeChild(generatorPanel.firstChild);
        }
    }
